...
NSString *const ASISRequestSchemeHTTP = @"http";
NSString *const ASISRequestSchemeHTTPS = @"https";
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setRequestScheme:ASISRequestSchemeHTTP];
	return self;
}
...
{
	[self setDateString:[[ASISRequest S3RequestDateFormatter] stringFromDate:date]];	
}

- (ASIHTTPRequest *)HEADRequest
{
	ASISRequest *headRequest = (ASISRequest *)[super HEADRequest];
...
- (void)buildRequestHeaders
{
	...
	if (![self accessKey]) {
		[self setAccessKey:[ASISRequest sharedAccessKey]];
	}
	if (![self secretAccessKey]) {
		[self setSecretAccessKey:[ASISRequest sharedSecretAccessKey]];
	}
	...
	NSString *signature = [ASIHTTPRequest base64forData:[ASISRequest HMACSHA1withKey:[self secretAccessKey] forString:stringToSign]];
...
{
	ASISRequest *newRequest = [super copyWithZone:zone];
...
{
	NSMutableDictionary *threadDict = [[NSThread currentThread] threadDictionary];
	NSDateFormatter *dateFormatter = [threadDict objectForKey:@"ASISRequestHeaderDateFormatter"];
	if (dateFormatter == nil) {
		dateFormatter = [[[NSDateFormatter alloc] init] autorelease];
		...
		[threadDict setObject:dateFormatter forKey:@"ASISRequestHeaderDateFormatter"];
	}
	return dateFormatter;
